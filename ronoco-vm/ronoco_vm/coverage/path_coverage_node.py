"""
Trajectory calculation algorithm for coverage from https://gitlab.com/Humpelstilzchen/path_coverage_ros
"""
# !/usr/bin/env python3
# -*- coding: iso-8859-15 -*-
...
class MapDrive(MarkerVisualization):
    def __init__(self, robot_width=0.3):
        ...
        self.move_base = actionlib.SimpleActionClient(config.move_base, MoveBaseAction)
    ...
    def do_boustrophedon(self, poly, costmap):
        ...
        rospy.loginfo("Converting costmap at x=%.2f..%.2f, y=%.2f %.2f for Boustrophedon Decomposition" % (
            minx, maxx, miny, maxy))
        ...
        with tempfile.NamedTemporaryFile(delete=False, mode='w') as ftmp:
            ...
            ftmp.flush()
            boustrophedon_script = os.path.join(self.rospack.get_path('ronoco'),
                                                "ronoco-vm/ronoco_vm/coverage/boustrophedon_decomposition.rb")
    ...
    def next_pos(self, x, y, angle):
        rospy.loginfo("Moving to (%f, %f, %.0fï¿œ)" % (x, y, angle * 180 / pi))
    ...
    def get_closes_possible_goal(self, pos_last, pos_next, angle, tolerance):
        ...
        for pose in plan.poses:
            ...
            cellx = round((
                                  local_pose.pose.position.x - self.local_costmap.info.origin.position.x) / self.local_costmap.info.resolution)
            celly = round((
                                  local_pose.pose.position.y - self.local_costmap.info.origin.position.y) / self.local_costmap.info.resolution)
    ...
    def drive_path(self, path):
        ...
        initial_pos = self.tfBuffer.lookup_transform(self.global_frame, self.base_frame, rospy.Time(),
                                                     rospy.Duration(0))
    ...
    def drive_polygon(self, polygon):
        ...
        rospy.logdebug("Rotated polygon by %.0fï¿œ: %s" % (angle * 180 / pi, str(poly_rotated.exterior.coords[:])))
