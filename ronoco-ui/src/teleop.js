...
async function forceStop() {
    let result = await post(url+":5000/teleop/force-stop", {})
...
async function left() {
    let result = await post(url+":5000/teleop/left", {})
    console.logger(result)
}
...
async function right() {
    let result = await post(url+":5000/teleop/right", {})
    console.logger(result)
}
...
async function forward() {
    let result = await post(url+":5000/teleop/forward", {})
    console.logger(result)
}
...
async function backward() {
    let result = await post(url+":5000/teleop/backward", {})
    console.logger(result)
}
